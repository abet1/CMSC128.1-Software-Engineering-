...
export function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-sm" style={{ bottom: 'max(20px, calc(env(safe-area-inset-bottom, 0px) + 8px))' }}>
      <div className="flex items-center justify-around bg-card border border-border/60 rounded-full px-3 py-3 shadow-2xl shadow-foreground/10">
        {navItems.map((item) => {
            ...
            <NavLink
              ...
              className={cn(
                ...
                isActive
                  ...
                  : 'text-muted-foreground/60 hover:text-muted-foreground hover:bg-muted/70'
              )}
            >
